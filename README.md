...
当前 `json` 只是稳定 CLI 形态的占位 envelope，不是最终 schema：
...
Roadmap 里的目标是 block-oriented JSON v1，包含 `blocks[]`、`kind`、`text`、`source_anchor`、`page`、`slide`、`sheet`、`row_range`、`truncated`、`warnings` 等字段。不要把 `pith-json-v0` 当作最终稳定 schema。
...
- 默认输出 LLM-friendly Markdown，后续提供可定位 JSON blocks。
...
- Block-oriented JSON v1：为 RAG/程序化调用提供稳定 blocks 和 source anchors。
